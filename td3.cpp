...
double* shrink_array(double* array, int length, int new_size) {
  // IMPLEMENT YOUR FUNCTION HERE
    ...
    for (int i = 0; i < length; i++){
        if(i < new_size) new_array[i] = array[i];
    }
    delete []array;
    return new_array;
}
  ...
  for (int i = 0; i < global_telemetry_current_size /3 - 1; i++){
        for (int j = 0; j < global_telemetry_current_size / 3 - i - 1; j++){
            if (global_telemetry[3*j] > global_telemetry[3*j + 3]){
                double temp1 = global_telemetry[3*j];
                double temp2 = global_telemetry[3*j + 1];
                double temp3 = global_telemetry[3*j + 2];
                global_telemetry[3*j + 1] = global_telemetry[3*j + 4];
                global_telemetry[3*j + 2] = global_telemetry[3*j + 5];
                global_telemetry[3*j + 3] = temp1;
                global_telemetry[3*j + 4] = temp2;
                global_telemetry[3*j + 5] = temp3;
            }
        }
